...
public class MumbleIceConnector {
    ...
    public static void logApiRequest(String msg)
    {
		...
		String filename = String.format("apilimit.%s.txt", dateFormat.format(date));
		...
		try {
			...
			count++;
			FileUtils.writeStringToFile(new File(filename), String.format("%d", count));
		} catch (IOException e) {
			e.printStackTrace();
		}
    ...
    {	
		...
		if (!file.exists()) {
			throw new Exception("apikey not found! place a file named \"apiKey.txt\" next to me with the apiKey in it!");
		}
    ...
    public static boolean checkApiLimit()
    {
		...
		try {
			String in = FileUtils.readFileToString(new File(filename));		
			MumbleIceConnector.logger.log(Level.WARNING, "WRITING: " + in);
